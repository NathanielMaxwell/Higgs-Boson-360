...
For the first dataset, we wanted to estimate the values of the coefficients $\boldsymbol{\beta}$ for each of the variables to find out how they impact whether or not a track is skipped. We are assuming little knowledge about each variable's effect, so we propose a weakly informative prior for $[\boldsymbol{\beta}]$: Using recommendations from Gelman, Jakulin, Pittau, and Su, we use a cauchy(0,2.5) prior for each scaled variable (we scaled the variables). Our response variable, $\textbf{y}$, will follow a logistic regression model, where 1 means the track was skipped. This is equivalent to the Bernoulli distribution $\textbf{y}|\theta \sim Bern(\theta)$. We will use the logit link, where $logit(\theta) = \eta$, and $\eta = \textbf{x}^T\boldsymbol{\beta}$, where $\textbf{x}$ is the covariate space for $\textbf{Y}$. Using the rstanarm package, Rstudio will compute the posterior and draw MCMC samples from the posterior distribution $[\boldsymbol{\beta}|\textbf{Y}, \textbf{X}$]. 
...
This model proved to be better, but not by much. To calculate our posterior predictive accuracy, we used the following method. If the posterior probability of a track being skipped  is greater or equal to $0.5$, then we would predict that observation to have a value of 1 (and similarly for less than $0.5$). For each observation, we can compare the posterior prediction to the actual observed value. The proportion of times we correctly predict an individual (i.e. [prediction = 0 and observation = 0] or [prediction = 1 and observation = 1]) is our classification accuracy. In our case, the posterior classification accuracy is 0.65. While we would want to also calculate the estimated accuracy on unseen data, since our number of observations is so large, we would expect the value to be the same when using a LOOCV, and this is indeed true: the value is still 0.65.
...
launch_shinystan(posterior3)
```

```{r, cache = T, echo=FALSE}
mcmc_areas(as.matrix(posterior3), prob = 0.90, prob_outer = 1)
...
#full_model <- stan_glm(target ~ acousticness+ danceability+ duration_ms+ energy+ instrumentalness+ liveness+ loudness+ mode+ speechiness+ tempo+ time_signature+ valence, data = spotify2,
#                 family = binomial(link = "logit"), 
#                 prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
 #                seed = seed,
  #               refresh = 0)
...
```{r, cache=TRUE}
posterior3.1 <- stan_glm(target ~ danceability+ duration_ms+ energy+ instrumentalness+ liveness+ loudness+ mode+ speechiness+ tempo+ time_signature+ valence, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
posterior3.2 <- stan_glm(target ~ acousticness+ duration_ms+ energy+ instrumentalness+ liveness+ loudness+ mode+ speechiness+ tempo+ time_signature+ valence, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
posterior3.3 <- stan_glm(target ~ acousticness+ danceability+ energy+ instrumentalness+ liveness+ loudness+ mode+ speechiness+ tempo+ time_signature+ valence, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
posterior3.4 <- stan_glm(target ~ acousticness+ danceability+ duration_ms+ instrumentalness+ liveness+ loudness+ mode+ speechiness+ tempo+ time_signature+ valence, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
posterior3.5 <- stan_glm(target ~ acousticness+ danceability+ duration_ms+ energy+ liveness+ loudness+ mode+ speechiness+ tempo+ time_signature+ valence, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
posterior3.6 <- stan_glm(target ~ acousticness+ danceability+ duration_ms+ energy+ instrumentalness+ loudness+ mode+ speechiness+ tempo+ time_signature+ valence, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
posterior3.7 <- stan_glm(target ~ acousticness+ danceability+ duration_ms+ energy+ instrumentalness+ liveness+ mode+ speechiness+ tempo+ time_signature+ valence, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
posterior3.8 <- stan_glm(target ~ acousticness+ danceability+ duration_ms+ energy+ instrumentalness+ liveness+ loudness+ speechiness+ tempo+ time_signature+ valence, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
posterior3.9 <- stan_glm(target ~ acousticness+ danceability+ duration_ms+ energy+ instrumentalness+ liveness+ loudness+ mode+ tempo+ time_signature+ valence, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
posterior3.10 <- stan_glm(target ~ acousticness+ danceability+ duration_ms+ energy+ instrumentalness+ liveness+ loudness+ mode+ speechiness+ time_signature+ valence, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
posterior3.11 <- stan_glm(target ~ acousticness+ danceability+ duration_ms+ energy+ instrumentalness+ liveness+ loudness+ mode+ speechiness+ tempo+ valence, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
posterior3.12 <- stan_glm(target ~ acousticness+ danceability+ duration_ms+ energy+ instrumentalness+ liveness+ loudness+ mode+ speechiness+ tempo+ time_signature, data = spotify1,
                family = binomial(link = "logit"), 
                prior = cauchy(0,2.5), prior_intercept = cauchy(0,2.5),
                seed = seed,
                refresh = 0)
...
(loo3.1 <- loo(posterior3.1, save_psis = T))
(loo3.2 <- loo(posterior3.2, save_psis = T))
(loo3.3 <- loo(posterior3.3, save_psis = T))
(loo3.4 <- loo(posterior3.4, save_psis = T))
(loo3.5 <- loo(posterior3.5, save_psis = T))
(loo3.6 <- loo(posterior3.6, save_psis = T))
(loo3.7 <- loo(posterior3.7, save_psis = T))
(loo3.8 <- loo(posterior3.8, save_psis = T))
(loo3.9 <- loo(posterior3.9, save_psis = T))
(loo3.10 <- loo(posterior3.10, save_psis = T))
(loo3.11 <- loo(posterior3.11, save_psis = T))
(loo3.12 <- loo(posterior3.12, save_psis = T))
```
```{r, cache=TRUE}
rstanarm::loo_compare(loo3, loo3.1, loo3.2, loo3.3, loo3.4, loo3.5, loo3.6, loo3.7, loo3.8, loo3.9, loo3.10, loo3.11, loo3.12)
```
```{r, cache=TRUE}
...
```{r, echo=FALSE, message=FALSE}
preds3 <- posterior_linpred(posterior3, transform = TRUE)
...
round(mean(xor(pr3,as.integer(spotify1$target == 0))),3)
```

```{r, cache = TRUE, echo=FALSE}
ploo3 = E_loo(preds3, loo3$psis_object, type="mean", log_ratios = -log_lik(posterior3))$value
round(mean(xor(ploo3>0.5,as.integer(spotify1$target==0))),3)
```
